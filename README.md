...
It sets out three scopes: **(A)** an enshrined delegation router that pools above a threshold must follow, enforced as a reward-eligibility rule (delegation off-target forfeits rewards) with a threshold schedule 11% → 8% → 5%; **(B)** a protocol-issued token, eFLR, under protocol custody; **(C)** both. The recommendation is A as the product, in five independent deliverables: machine-readable disclosure first (Scope 0: entity identity, self-bond source, reward accounting, one public check); a percentage cap per entity in FIP.02/FIP.05; a non-binding allocation oracle; A as a reward filter, Songbird-tested; and contract-originated P-chain delegation as its own FIP, not hung on the FIP.16 builder roadmap. B only behind that FIP or a reviewed FCC key set with the custody matrix filled in; otherwise, explicitly, not.
