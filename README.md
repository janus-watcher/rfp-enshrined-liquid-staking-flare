...
The router allocates by trust weight × net-yield weight, capped per entity as a share of *network* stake so the protocol's own pool is incapable of concentrating. Validator competition is not softened by this; it moves from relationships with pool operators to uptime, data quality and fee, on terms every entity can see. There is no slashing on Flare's P-chain, so there is no "insurance fund"; there is a redemption reserve with a contract invariant and an exit queue. A protocol fee, proposed at 5% and open to review, goes to the reserve until its invariant is met, then to FIRE. Native PT/YT stripping is recommended against; Spectra rails already exist. Failure modes lead with the strongest: an enshrined pool is itself the largest pool on the network, and its neutrality is exactly as good as the formula and the keys behind it.
